        ...
        try {
            if ($_SERVER["REQUEST_METHOD"] == "PUT") {
                ...
                if (isset($input['reservationId'], $input['firstName'], $input['lastName'], $input['email'], $input['phoneNumber'], $input['session'], $input['date'], $input['numberOfAdults'], $input['numberOfChildren'], $input['comment'], $input['active'])) {
                    ...
                    $firstName = HandleDataCheck::filterEmptyStringAPI($input['firstName']);
                    $lastName = HandleDataCheck::filterEmptyStringAPI($input['lastName']);
                    $email = HandleDataCheck::filterEmptyStringAPI($input['email']);
                    $phoneNumber = HandleDataCheck::filterEmptyStringAPI($input['phoneNumber']);
                    ...
                    $comment = HandleDataCheck::filterEmptyStringAPI($input['comment']);
        ...
        try {
            if ($_SERVER["REQUEST_METHOD"] == "POST") {
                ...
                if (isset($input['restaurantName'], $input['firstName'], $input['lastName'], $input['email'], $input['phoneNumber'], $input['session'], $input['date'], $input['numberOfAdults'], $input['numberOfChildren'])) {
                    $restaurantName = HandleDataCheck::filterEmptyStringAPI($input['restaurantName']);
                    $firstName = HandleDataCheck::filterEmptyStringAPI($input['firstName']);
                    $lastName = HandleDataCheck::filterEmptyStringAPI($input['lastName']);
                    $email = HandleDataCheck::filterEmptyStringAPI($input['email']);
                    $phoneNumber = HandleDataCheck::filterEmptyStringAPI($input['phoneNumber']);
    ...
    public function getSessionByRestaurantName()
    {
        try {
            if ($_SERVER["REQUEST_METHOD"] == "GET" && isset($_GET['name'])) {
                HandleDataCheck::filterEmptyStringAPI($restaurantName = $_GET['name']);
