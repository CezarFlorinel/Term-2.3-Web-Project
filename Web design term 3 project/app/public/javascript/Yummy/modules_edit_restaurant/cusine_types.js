...
function updateCuisineDisplay() {
    try {
        ...
        errorHandler.logError(error, 'updateCuisineDisplay', 'cuisine_types.js');
        errorHandler.showAlert('An error occurred while updating the cuisine types, please try again later!');
    }
}
...
function sendCuisineUpdate() {
    ...
    fetch('/api/restaurantIndividualAdmin/updateRestaurantCuisineTypes', {
        ...
        .catch((error) => {
            errorHandler.logError(error, 'sendCuisineUpdate', 'cuisine_types.js');
            errorHandler.showAlert('An error occurred while updating the cuisine types, please try again later!');
        });
}
...
export function addCuisine() {
    ...
    addButton.addEventListener('click', function () {
        ...
        let newType = newTypeInput.value.trim(); // Get the new type from the input field and trim any whitespace
        newType = newType.replace(/;/g, ''); // Remove any semicolons from the input
        if (!checkText({ newType })) {
            return;
        }
